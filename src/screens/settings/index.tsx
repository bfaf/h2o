...
} from '../../stores/redux/thunks/settings';
...
export const Settings = (): React.JSX.Element => {
  ...
  const onSelectedCoffeeCupMenuItem = (
    ...
    const setWaterPerCoffee = async (): Promise<void> => {
      switch (value) {
        case '100 ml':
        ...
        case '300 ml':
          ...
          break;
        case '400 ml':
          await dispatch(setWaterPerCoffeeCup(400));
          break;
        case '500 ml':
          await dispatch(setWaterPerCoffeeCup(500));
          break;
        default:
  ...
  const onToggleIconSwitch = () => {
    dispatch(setHumanIcon(!femaleIcon));
  };
  const onToggleSwitch = () => {
    dispatch(setReminderSwitch(!remindersToggleEnabled));
  };
  ...
  const renderCard = (amount: string, idx: number) => {
    return (
      <Card key={`${idx}-${amount}`} mode="outlined" style={{ margin: 5 }}>
        <Card.Title
          title={`${amount}ml`}
          right={(props) => (
            <IconButton
      ...
      <ScrollView>
        <View style={styles.container}>
          <View style={styles.reminderSwitchContainer}>
            ...
            <Menu
              visible={showCoffeeCupMenu}
              onDismiss={closeCoffeeMenu}
              anchor={
                <Button mode="outlined" labelStyle={styles.biggerText} onPress={openCoffeeMenu}>
                  {waterPerCoffeeCup} ml
                </Button>
              }
            >
          ...
          <View style={styles.reminderSwitchContainer}>
            ...
            <Switch
              ...
              value={femaleIcon}
              onValueChange={onToggleIconSwitch}
            />
          </View>
          <View style={styles.reminderSwitchContainer}>
            ...
            <Switch
              style={styles.reminderSwitch}
              value={remindersToggleEnabled}
              onValueChange={onToggleSwitch}
            />
          </View>
          <View style={styles.reminders}>
            <View style={styles.timeManagement}>
              <Text disabled={!remindersToggleEnabled} style={styles.reminderFromText}>
                From:{' '}
              </Text>
              <Button
              ...
              <Text disabled={!remindersToggleEnabled} style={styles.reminderToText}>
                To:{' '}
              </Text>
              <Button
                disabled={!remindersToggleEnabled}
            ...
            <View style={styles.repeatInterval}>
              ...
              <Menu
                visible={showRepeatIntervalMenu}
                onDismiss={closeMenu}
                anchor={
                  <Button
                    mode="outlined"
          ...
          <View key="waterAmounts" style={styles.waterAmountsContainer}>
            <Text key="waterTitle" style={styles.waterAmountsTitle}>
              Water amounts
            </Text>
            <Text key="waterDescriptionText" style={styles.waterAmountsDescriptionText}>
              Add up to 3 water amounts as you desire
            </Text>
          ...
          <View key="destructive-actions" style={{ padding: 25 }}>
            <Button
              ...
              labelStyle={{ ...styles.biggerText, color: 'white' }}
              onPress={() => {
                dispatch(deleteAllHistoryData());
              }}
            >
              Clear history data
            </Button>
