...
export const createTable = async (
  ...
  const columns = columnConfig
    .map((column) => `${column.name} ${column.type} ${column.canBeNull ? '' : 'NOT NULL'}`)
    .join(', ');
...
export const insertDataToTableTransactional = async (
  db: Transaction,
  tableName: string,
  data: Record<string, any>,
) => {
...
export const updateValue = async <T>(
  value: any,
  prop: keyof T,
  stateProp: keyof RootState,
  tableName: string,
  { rejectWithValue }: GetThunkAPI<any>,
) => {
  let db = undefined;
  try {
